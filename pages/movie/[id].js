                ...
                <section className="relative  z-50">
                    ...
                    >
                        <div className="flex items-center justify-between bg-black text-[#f9f9f9] p-3.5">
                            ...
                            <div
                                className="cursor-pointer w-8 h-8 flex items-center justify-center rounded-lg opacity-50 hover:opacity-75 hover:bg-[#0F0F0F]" onClick={() => setShowPlayer(false)}
                            >
                                <XIcon className="h-5" />
                            </div>
                        </div>
...
export async function getServerSideProps(context) {
    ...
    const request = await fetch(`https://api.themoviedb.org/3/movie/${id}?api_key=${process.env.API_KEY}&language=en-US&&append_to_response=videos`).then(
